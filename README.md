...
## Reproduce steps of running fuse in the init process.
```
CGO_ENABLED=0 go build -o loopback .
sudo docker build -t fuse-deadlock:latest .
sudo docker run --name=fuse-as-init-process  --rm --privileged  -it -v /dev/fuse:/dev/fuse fuse-deadlock:latest 
# in another terminal
docker exec -it fuse-as-init-process sh
...
docker stop fuse-as-init-process -s SIGKILL
ps -ef | grep loopback
...
## Reproduce steps of running fuse in the child process of the init process
```bash
sudo docker run --name=fuse-as-child-process  --rm --privileged  -it -v /dev/fuse:/dev/fuse fuse-deadlock:latest  /loopback --run-fuse-in-child-process --mountpoint=/tmp/mp --backend=/root/backend 
# in another terminal
docker exec -it fuse-as-child-process sh
# wait the cat running, it will be launched after 5 second on fuse start.
cat /proc/${PID_OF_CAT}/stack
cat /proc/${PID_OF_CAT}/status
kill ${PID_OF_CAT}
# after send SIGTERM to the cat,insepct its stack and status again, it still stuck at request_wait_answer and its status becomes (D) disk sleep
cat /proc/${PID_OF_CAT}/status
cat /proc/${PID_OF_CAT}/stack

# Now we send SGIKILL to container, the container will become zombie and stuck at `do_exit` function.
# Although the fuse process exited successfully, but the init process is waiting child process `cat` to exit,the child process is waiting the fuse response, the deadlock happens.

docker stop fuse-as-child-process -s SIGKILL
ps -ef | grep loopback
kill -9 {PID_OF_LOOPBACK}
ls /proc/${PID_OF_LOOPBACK}/task
# There is one task that stucked at `do_exit`
